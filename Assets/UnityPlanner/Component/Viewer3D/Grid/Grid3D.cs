...
[Serializable]
public class Grid3D : MonoSingleton<RTSceneGrid>, IXZGrid
{
    ...
    // 是否弹起鼠标左键
    public bool isLeftButtonUp = false;
    // 起始点
    ...
    void Update()
    {
        var gridRayHit = RaycastSceneGridIfVisible(Camera.main.ScreenPointToRay(Input.mousePosition));
        if (gridRayHit != null)
        {
            var HitCell = gridRayHit.HitCell;
            HitPoint = new Vector3(Mathf.RoundToInt(gridRayHit.HitPoint.x), Mark1.transform.position.y, Mathf.RoundToInt(gridRayHit.HitPoint.z));
            if (Input.GetMouseButtonDown(0))
            {
                StartPosition = HitPoint;
                Debug.Log(string.Format("记录起始点{0}",StartPosition));
            }

            if (Input.GetMouseButtonUp(0))
            {
                EndPosition = HitPoint;
                Debug.Log(string.Format("记录结束点{0}",EndPosition));
            }

            Mark1.transform.position = HitPoint;
            Mark2.transform.position = gridRayHit.HitCell.Center;
            // if (gridRayHit.HitCell.GO == null)
            // {
            //     gridRayHit.HitCell.GO = GameObject.Instantiate(Spaw1,gridRayHit.HitCell.Center,Quaternion.identity);
            // }
            // else
            // {
            //     Debug.Log(string.Format("{0}:{1}", gridRayHit.HitCell.Center.ToString(), "已经存在对象"));
            // }
        }
    }

    public XZGridCell GetXZGridCell(Ray ray)
    {
        ...
        {
            XZGridCell hitCell = CellFromWorldPoint(ray.GetPoint(t));
            return hitCell;
        }
