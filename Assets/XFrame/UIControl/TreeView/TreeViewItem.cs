...
namespace XTreeView
{
    ...
    public class TreeViewItem : MonoBehaviour
    {
        ...
        public Button BtnExpanding;
        public Text TreeViewText;
        public Text TreeViewCountText;
        //是否显示
        ...
        void ChangeChildrenToggle(TreeViewItem tvi, bool value)
        {
            for (int i = 0; i < tvi.GetChildrenNumber(); i++)
            {
                tvi.GetChildrenByIndex(i).TreeViewToggle.isOn = value;
                ChangeChildrenToggle(tvi.GetChildrenByIndex(i), value);
            }
        }

        private List<string> GetAllChildrensId(TreeViewItem tvi)
        {
            var list = new List<string> {tvi.Data.Id};
            for (int i = 0; i < tvi.GetChildrenNumber(); i++)
            {
                list.AddRange(GetAllChildrensId(tvi.GetChildrenByIndex(i)));
            }
