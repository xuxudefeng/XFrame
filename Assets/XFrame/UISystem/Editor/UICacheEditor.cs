...
[CustomEditor(typeof(UICache), true)]
public class UICacheEditor : UnityEditor.Editor
{
    ...
    private const string MenuItemName = "GameObject/复制模板代码";
    private const int MenuItemPriority = 48;

    [MenuItem("GameObject/UIManager", false, MenuItemPriority)]
    ...
    public static bool CopyGameTemplateValidate()
    {
    ...
    public static void CopyGameTemplate()
    {
    ...
    private static (string Body, string TemplateName) GenerateTemplate(IMapper uiCache, UIStringElement[] stringElements)
    {
        ...
        var classNameTemplate = CopyTemplateSettings.DefaultClassName;
        var targetTypesTemplate = CopyTemplateSettings.DefaultPickupComponentNames;
        var removeText = CopyTemplateSettings.DefaultRemoveText;

        var guids = AssetDatabase.FindAssets($"t:{nameof(CopyTemplateSettings)}");
        if (guids.Length > 0)
        {
            var path = AssetDatabase.GUIDToAssetPath(guids[0]);
            var settings = AssetDatabase.LoadAssetAtPath<CopyTemplateSettings>(path);

            templateName = path;
            classNameTemplate = settings.ClassName ?? CopyTemplateSettings.DefaultClassName;
        ...
        {
            if (e.Path.Length == 0) continue;// 根据tag筛选
            foreach (var component in e.GameObject.GetComponents<Component>())
            {
                ...
                while (uniquePath.Length > 1)
                {
                    ...
                    if (count == 0) throw new Exception($"{findPath} 路径未找到");
                    if (count != 1) break;
                    uniquePath = next;
                }
